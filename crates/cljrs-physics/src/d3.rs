//! 3D physics bindings via rapier3d. Mirrors `d2` module; only vector
//! dimensionality differs (all [x y] become [x y z]).
...
fn need3(c: &[f32], name: &str, field: &str) -> Result<[f32; 3]> {
    if c.len() != 3 {
        return Err(Error::Type(format!(
            "{name}: {field} must be [x y z], got {} components",
            c.len()
        )));
    }
    Ok([c[0], c[1], c[2]])
...
fn world_fn(args: &[Value]) -> Result<Value> {
    let gravity = if args.is_empty() {
        ...
        match map_get(m, "gravity") {
            Some(v) => {
                let c = need3(&vec_components(v)?, "world", ":gravity")?;
                vector![c[0], c[1], c[2]]
            }
            None => vector![0.0, -9.81, 0.0],
        }
    };
    Ok(opaque(TAG, World3::new(gravity)))
}

fn add_body_fn(args: &[Value]) -> Result<Value> {
    let w = arg_world::<World3>(args, 0, TAG, "add-body!")?;
    ...
    let position = match map_get(m, "position") {
        Some(v) => need3(&vec_components(v)?, "add-body!", ":position")?,
        None => [0.0, 0.0, 0.0],
    };
    let linvel = match map_get(m, "linvel") {
        Some(v) => need3(&vec_components(v)?, "add-body!", ":linvel")?,
        None => [0.0, 0.0, 0.0],
    };
    ...
    let builder = match shape.as_str() {
        "ball" => {
            let r = match map_get(m, "radius") {
                Some(v) => as_f32(v)?,
                None => return Err(Error::Type("collider :ball needs :radius".into())),
            };
            ColliderBuilder::ball(r)
        }
        "box" | "cuboid" => {
            let half = match map_get(m, "half-extents") {
                Some(v) => need3(&vec_components(v)?, "collider", ":half-extents")?,
...
fn step_fn(args: &[Value]) -> Result<Value> {
    let w = arg_world::<World3>(args, 0, TAG, "step!")?;
    w.lock().unwrap().step();
    Ok(Value::Nil)
}
...
fn with_body<F, R>(args: &[Value], name: &str, f: F) -> Result<R>
where
    F: FnOnce(&mut World3, RigidBodyHandle) -> Result<R>,
{
    let w = arg_world::<World3>(args, 0, TAG, name)?;
...
fn set_linvel_fn(args: &[Value]) -> Result<Value> {
    let vel = match args.get(2) {
        Some(v) => {
            let c = need3(&vec_components(v)?, "set-linvel!", "velocity")?;
            vector![c[0], c[1], c[2]]
        }
...
fn apply_impulse_fn(args: &[Value]) -> Result<Value> {
    let imp = match args.get(2) {
        Some(v) => {
            let c = need3(&vec_components(v)?, "apply-impulse!", "impulse")?;
            vector![c[0], c[1], c[2]]
        }
...
fn body_count_fn(args: &[Value]) -> Result<Value> {
    let w = arg_world::<World3>(args, 0, TAG, "body-count")?;
